...
public class Marshalling {
    ...
    public static String[] recursiveGetDBusType(Type c, boolean basic, int level) throws DBusException {
        ...
        if (c instanceof TypeVariable)
            ...
            out[level].append((char) Message.ArgumentType.OBJECT_PATH);
        else if (c instanceof Class && Path.class.equals((Class<? extends Object>) c))
            out[level].append((char) Message.ArgumentType.OBJECT_PATH);
        else if (c instanceof Class && ObjectPath.class.equals((Class<? extends Object>) c))
            out[level].append((char) Message.ArgumentType.OBJECT_PATH);
     ...
     */
    @SuppressWarnings("unchecked")
    public static Object[] convertParameters(Object[] parameters, Type[] types, AbstractConnection conn)
    ...
    static List<Object> deSerializeParameters(List<Object> parameters, Type type, AbstractConnection conn)
        ...
        for (int i = 0; i < parameters.size(); i++) {
             ...
             * throw new DBusException("Not enough elements to create custom object from serialized data ("
             * +(parameters.size()-i)+" < "+(newtypes.length)+")");
             * }
             * }
             * } else
             */
