 ...
 * @author Karel Goderis - added simple polling mechanism
 */
...
public class UpnpIOServiceImpl implements UpnpIOService {
						...
						if (service != null) {
							...
							if (action != null) {
								...
								if (invocation != null) {
									...
									logger.debug("Polling participant '{}' through Action '{}' of Service '{}' ", new Object[] {participant.getUDN(),
											actionID, serviceID  });
									new ActionCallback.Default(invocation, upnpService.getControlPoint()).run();
	...
	public void addStatusListener(UpnpIOParticipant participant, String serviceID, String actionID, int interval) {
		...
		if(participant != null) {
			...
			pollingJobs.put(participant, scheduler.scheduleAtFixedRate(pollingRunnable, 0, pollingInterval, TimeUnit.SECONDS));
