...
public class AutomationCommandList extends AutomationCommand {
  ...
  protected String parseOptionsAndParameters(String[] params) {
    ...
    for (int i = 0; i < params.length; i++) {
        ...
        if (getId) return String.format("[Automation Commands : Command \"%s\"] Unsupported parameter: %s", command, params[i]);
      }
      return SUCCESS;
    }
    ...
    if (collection != null && !collection.isEmpty()) {
      ...
      if (id != null) {
        ...
        if (collection.size() == 1) {
          ...
          if (!templates.isEmpty()) templates.clear();
          addCollection(collection, templates);
