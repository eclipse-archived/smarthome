...
public class Printer {
   ...
   * @param command 
   */
  static String print(Hashtable<String, String> list, String command, String id) {
    StringBuilder writer = new StringBuilder();
    if (list.isEmpty()) {
      writer.append("[Automation Commands : Command \"" + command + "\"] List is empty.");
    } else {
      writer.append("\nID");
   ...
   * @param rule
   */
  static String printRule(Rule rule) {
    StringBuilder writer = new StringBuilder();
    printChars(writer, '-', 100, true);
    ...
    if (triggers != null && !triggers.isEmpty()) {
      ...
      for (Trigger trigger : triggers) {
        ...
        last--;
        if (last > 0) printChars(writer, ' ', 30, false);
      }
    }
   ...
   * @param writer
   */
  private static String printConfigurationDescription(Set<ConfigDescriptionParameter> cd) {
    StringBuilder writer = new StringBuilder();
    Iterator i = cd.iterator();
    ConfigDescriptionParameter parameter = (ConfigDescriptionParameter) i.next();
    writer.append(parameter.getName() + "\n");
    writer.append(printParameter(parameter));
    while (i.hasNext()) {
      parameter = (ConfigDescriptionParameter) i.next();
      printChars(writer, ' ', 30, false);
      writer.append(parameter.getName() + "\n");
      writer.append(printParameter(parameter));
    }
    ...
    while (i.hasNext()) {
      ...
      if (element instanceof String) {
        ...
        if (i.hasNext()) {
          ...
          res = ((Connection) element).getInputName() + " = "
              + ((Connection) element).getOuputModuleId() + "." + ((Connection) element).getOutputName();
        }
      }
    }
  ...
  private static void printChars(StringBuilder writer, char ch, int count, boolean nl) {
    if (count < 1) {
      return;
    }
