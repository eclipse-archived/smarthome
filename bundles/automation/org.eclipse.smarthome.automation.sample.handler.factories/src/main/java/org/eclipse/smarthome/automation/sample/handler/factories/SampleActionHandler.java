...
public class SampleActionHandler implements ActionHandler {
  ...
  public SampleActionHandler(SampleHandlerFactory handlerFactory, Action action, ActionType actionType, BundleContext bc) {
    this.action = action;
  ...
  public Map<String, Object> execute(Map<String, ?> inputs) {
    ...
    System.out.println("[Automation demo] " + actionType.getUID() + "/" + action.getId() + ": " +
        prefix + " " + inputData);
    return null;
  }
}
