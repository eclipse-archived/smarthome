...
public class SampleHandlerFactoryCommands extends AbstractConsoleCommandExtension {
	...
	public void execute(String[] args, Console console) {
		...
		if (COMMAND_LIST.equalsIgnoreCase(command )
				|| "ls".equalsIgnoreCase(command)) {
			listTriggerSpi(params, console);
		} else if (COMMAND_EXECUTE.equalsIgnoreCase(command)
				|| "ex".equalsIgnoreCase(command)) {
	...
	public List<String> getUsages() {
		return Arrays.asList(new String[] {
				buildCommandUsage(COMMAND_LIST, "List all created TriggerSpi"),
