...
public class Introspector {
  ...
  Object deserialize(Object jsonData, Class toType) throws JSONException {
    ...
    final Class primClass = (Class) primitiveMap.get(toType);
    if (primClass != null) {
      toType = primClass;
    }
  ...
  private Method findPublicMethod(String name, Class returnType, Class param, Class fromClass) {
    ...
    for (int j = 0; j < methods.length; j++) {
      ...
      if (methods[j].getName().equals(name) &&
          ...
          (param == null && paramTypes.length == 0 || paramTypes.length == 1 && paramTypes[0].equals(param))) {
        return methods[j];
      }
    }
    return null;
