    ...
    try {
      ...
      if (jsonRuleTemplates != null) {
        if (jsonRuleTemplates instanceof JSONArray) {
          ...
          ruleTemplatesStatus.add(createRuleTemplate((JSONObject) jsonRuleTemplates,
              new Status(log, Status.TEMPLATE, null)));
        }
      }
 ...
 * @see org.eclipse.smarthome.automation.handler.parser.TemplateParser#writeRuleTemplates(org.eclipse.smarthome.automation.template.RuleTemplate, java.io.OutputStreamWriter)
   */
  ...
  */
  ...
  private Status createRuleTemplate(JSONObject jsonRuleTemplate, Status status) {
    ...
    if (sectionTrigers == null)
      return status;
    JSONArray sectionConditions = JSONUtility.getJSONArray(JSONStructureConstants.IF, true, jsonRuleTemplate, status);
    JSONArray sectionActions = JSONUtility.getJSONArray(JSONStructureConstants.THEN, false, jsonRuleTemplate, status);
    ...
    if (!ModuleJSONParser.createTrigerModules(status, moduleManager, automationFactory, null, triggers, sectionTrigers))
      return status;
    ModuleJSONParser
        .createConditionModules(status, moduleManager, automationFactory, null, conditions, sectionConditions);
    ...
    String visibilityString = JSONUtility.getString(JSONStructureConstants.VISIBILITY, true, jsonRuleTemplate, status);
    ...
    RuleTemplate template = new RuleTemplate(ruleTemplateUID, triggers, conditions, actions,
