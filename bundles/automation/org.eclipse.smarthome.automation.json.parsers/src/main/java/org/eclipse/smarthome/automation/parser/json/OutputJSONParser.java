...
public class OutputJSONParser {
  ...
  static Output createOutput(BundleContext bc, String outputName, JSONObject jsonOutputDescription, Status status) {
    ...
    String description = JSONUtility.getString(JSONStructureConstants.DESCRIPTION, true, jsonOutputDescription, status);
    String reference = JSONUtility.getString(JSONStructureConstants.REFERENCE, false, jsonOutputDescription, status);
    if (reference == null)
      return null;
    ...
    return new Output(outputName, type, label, description, tags, reference,
        convDefaultValue);
  }
   ...
   * @return
   */
  static Map<String, Output> convertOutputToMap(Set<Output> outputs) {
    if (outputs == null) {
      return null;
    }
