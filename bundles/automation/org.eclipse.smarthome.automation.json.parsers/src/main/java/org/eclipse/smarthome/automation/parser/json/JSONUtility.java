...
public class JSONUtility {
   ...
   */
  static void verifyType(BundleContext bc, String outputType, String inputType) throws IllegalArgumentException,
      ClassNotFoundException {
    Class<?> clazzOutput = bc.getBundle().loadClass(outputType);
    Class<?> clazzInput = bc.getBundle().loadClass(inputType);
    if (clazzOutput.isAssignableFrom(clazzInput)) {
      return;
    }
    throw new IllegalArgumentException("Incompatible types : \"" + outputType + "\" and \"" + inputType + "\".");
  }
   ...
   */
  static Object convertValue(BundleContext bc, String type, Object value) throws ClassNotFoundException, JSONException,
  ...
  static String getString(String key, int index, JSONArray jsonArray, Status status) {
    ...
    if (element == null || !(element instanceof String)) {
      status.error("Elements of \"" + key + "\" must be present and must be Strings in : " + element, new IllegalArgumentException());
      return null;
    }
  ...
  static JSONObject getJSONObject(String key, int index, JSONArray jsonArray, Status status) {
    ...
    if (element == null || !(element instanceof JSONObject)) {
      status.error("Elements of \"" + key + "\" must be present and must be JSONObjects in : " + element, new IllegalArgumentException());
      return null;
    }
   ...
   * @return
   */
  static boolean isOptionalConfig(Set<ConfigDescriptionParameter> configDescriptions) {
    if (configDescriptions != null && !configDescriptions.isEmpty()) {
      boolean required = false;
      Iterator<ConfigDescriptionParameter> i = configDescriptions.iterator();
      while (i.hasNext()) {
        ConfigDescriptionParameter param = (ConfigDescriptionParameter) i.next();
