...
public class InputJSONParser {
  ...
  static Input createInput(BundleContext bc, String inputName, JSONObject jsonInputDescription, Status status) {
    ...
    String description = JSONUtility.getString(JSONStructureConstants.DESCRIPTION, true, jsonInputDescription, status);
    ...
    return new Input(inputName, type, label, description, tags, required, reference,
        convDefaultValue);
  }
   ...
   * @return
   */
  static Map<String, Input> convertInputToMap(Set<Input> inputs) {
    if (inputs == null) {
      return null;
    }
