...
class TriggerTypeJSONParser {
   ...
   * @throws JSONException 
   */
  static void writeTriggerTypes(Map<String, TriggerType> triggers, OutputStreamWriter writer) throws IOException, JSONException {
    if (!triggers.isEmpty()) {
      writer.write(" " + JSONStructureConstants.TRIGGERS + ":{\n");
      Iterator<String> triggersI = triggers.keySet().iterator();
      while (triggersI.hasNext()) {
        String triggerUID = (String) triggersI.next();
        TriggerType trigger = (TriggerType) triggers.get(triggerUID);
