...
public class MultipleTemplateFileProvider extends AbstractMultipleFileProvider<RuleTemplate, TemplateFileProvider>
            ...
            if (providers != null) {
                ...
                for (int i = 0; i < this.roots.length; i++) {
                    if (!providers.containsKey(this.roots[i])) {
                        TemplateFileProvider provider = new TemplateFileProvider(this.roots[i]);
    ...
    public void activate(Map<String, Object> config) {
        modified(config);
        providers = new HashMap<String, TemplateFileProvider>(this.roots.length);
        for (int i = 0; i < roots.length; i++) {
            TemplateFileProvider provider = new TemplateFileProvider(roots[i]);
            provider.addProviderChangeListener(this);
            provider.activate(parsers);
