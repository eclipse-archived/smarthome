...
public abstract class AbstractFileProvider<E> extends AbstractWatchService implements Provider<E> {
    ...
    protected void removeResources(File file) {
        try {
            URL url = file.toURI().toURL();
            synchronized (providerPortfolio) {
                List<String> portfolio = providerPortfolio.remove(url);
                if (file.isDirectory()) {
    ...
    protected void importFile(String parserType, URL url) {
        ...
        if (parser != null) {
            try (InputStream is = url.openStream();
                    InputStreamReader inputStreamReader = new InputStreamReader(new BufferedInputStream(is));) {
    ...
    protected void removeElements(List<String> objectsForRemove) {
        if (objectsForRemove != null) {
            for (String removededObject : objectsForRemove) {
                notifyListeners(providedObjectsHolder.remove(removededObject));
            }
        }
    }
    ...
    private static class WatchQueueReader extends AbstractWatchQueueReader {
        ...
        protected void processWatchEvent(WatchEvent<?> event, Kind<?> kind, Path path) {
            if (kind.equals(ENTRY_DELETE)) {
                provider.removeResources(new File(provider.getSourcePath() + File.separator + path.toString()));
            } else {
