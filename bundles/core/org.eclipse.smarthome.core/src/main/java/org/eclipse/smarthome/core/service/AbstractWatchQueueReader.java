...
public abstract class AbstractWatchQueueReader implements Runnable {
	...
	public void run() {
		try {
			for (;;) {
				...
				try {
					...
					logger.warn(MessageFormat.format(
							"Catched InterruptedException : {0}",
				...
				for (WatchEvent<?> event : key.pollEvents()) {
					...
					if (kind == OVERFLOW) {
						logger.warn(MessageFormat.format(
								"Found event with overflow kind : {0}", event));
			...
			logger.debug(
					...
					ecx.getLocalizedMessage(), Thread.currentThread().getName());
	...
	protected abstract void processWatchEvent(WatchEvent<?> event,
			WatchEvent.Kind<?> kind, Path path);
}