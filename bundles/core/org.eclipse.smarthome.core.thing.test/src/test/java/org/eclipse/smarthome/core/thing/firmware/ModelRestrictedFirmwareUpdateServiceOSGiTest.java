...
import static org.eclipse.smarthome.core.thing.firmware.FirmwareStatus.*;
...
public class ModelRestrictedFirmwareUpdateServiceOSGiTest extends JavaOSGiTest {
    ...
    private FirmwareProvider createFirmwareProvider(Firmware... firmwares) {
        return new FirmwareProvider() {
            ...
            public Firmware getFirmware(FirmwareUID firmwareUID, Locale locale) {
                for (Firmware firmware : getFirmwares(firmwareUID.getThingTypeUID(), locale)) {
                    if (firmware.getUID().equals(firmwareUID)) {
                        return firmware;
                    }
                }
                return null;
    ...
    private Firmware createModelRestrictedFirmware(String model, String version) {
        return new Firmware.Builder(new FirmwareUID(thingType.getUID(), version)).withModel(model)
                .withModelRestricted(true).build();
    }

    @SuppressWarnings("null")
    private void assertThatThingHasFirmware(ThingUID thingUID, String firmwareVersion) {
        assertThat(thingRegistry.get(thingUID).getProperties().get(PROPERTY_FIRMWARE_VERSION), is(firmwareVersion));
    }
