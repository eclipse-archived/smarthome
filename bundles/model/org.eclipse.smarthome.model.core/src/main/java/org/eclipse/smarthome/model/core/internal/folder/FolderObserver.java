...
public class FolderObserver extends AbstractWatchService implements
	...
	protected AbstractWatchQueueReader buildWatchQueueReader(
		...
		return new WatchQueueReader(watchService, toWatch, folderFileExtMap,
				modelRepo, configDispatcher);
	}

	@Override
	protected String getSourcePath() {
		return configDispatcher.getConfigFolder();
	}
	...
	private static class WatchQueueReader extends AbstractWatchQueueReader {
		...
		public WatchQueueReader(WatchService watchService, Path dirToWatch,
				...
				ModelRepository modelRepo, ConfigDispatcher configDispatcher) {
		...
		protected void processWatchEvent(WatchEvent<?> event, Kind<?> kind,
				Path path) {
			File toCheck = getFileByFileExtMap(configDispatcher,
					folderFileExtMap, path.toString());
			if (toCheck != null) {
	...
	public synchronized void updated(Dictionary config)
		...
		if (config != null) {
			...
			while (keys.hasMoreElements()) {
				...
				File folder = getFile(configDispatcher, foldername);
	...
	private static File getFileByFileExtMap(ConfigDispatcher configDispatcher,
			...
			if (StringUtils.isNotBlank(extension)) {
				...
				while (iterator.hasNext()) {
					...
					if (ArrayUtils.contains(entry.getValue(), extension)) {
						return new File(getFile(configDispatcher,
								entry.getKey())
	 ...
	 */
	private static File getFile(ConfigDispatcher configDispatcher,
			String filename) {
