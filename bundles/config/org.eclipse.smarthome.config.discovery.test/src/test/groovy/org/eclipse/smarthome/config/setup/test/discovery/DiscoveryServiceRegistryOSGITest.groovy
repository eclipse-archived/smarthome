...
class DiscoveryServiceRegistryOSGITest extends OSGiTest {
    ...
    def ANY_BINDING_ID_3 = 'anyBindingId3'
    def ANY_THING_TYPE_3 = 'anyThingType3'

    def BRIDGE_UID_1 = new ThingUID("binding:bridge:1")
    def BRIDGE_UID_2 = new ThingUID("binding:bridge:2")
    ...
    DiscoveryServiceMockOfBridge discoveryServiceMockForBinding3Bridge1
    DiscoveryServiceMockOfBridge discoveryServiceMockForBinding3Bridge2
    ...
    void setUp() {
        ...
        serviceRegs.add(registerService(discoveryServiceMockForBinding3Bridge1, DiscoveryService.class.name))
        serviceRegs.add(registerService(discoveryServiceMockForBinding3Bridge2, DiscoveryService.class.name))
    ...
    void 'assert that an listener is notified about removeOlderResults' () {
        ...
        DiscoveryListener discoveryListenerMock = [
            removeOlderResults: { DiscoveryService src, long timestamp, Collection<ThingTypeUID> thingTypeIds, ThingUID bridgeUID ->
    ...
    void 'assert that removeOlderResults works as expected' () {
        ...
        DiscoveryListener discoveryListenerMock = [
            ...
            removeOlderThings: { DiscoveryService source, long timestamp, Collection<ThingTypeUID> thingTypeUIDs, ThingUID bridgeUID ->
                []
            }
        ] as DiscoveryListener
    ...
    @Test
    void 'assert that removeOlderResults removes only the entries of the specified bridge' () {
        AsyncResultWrapper<Boolean> scanListenerResult3 = new AsyncResultWrapper<Boolean>()
        AsyncResultWrapper<DiscoveryResult> discoveryListenerResult = new AsyncResultWrapper<Boolean>()
        DiscoveryListener discoveryListenerMock = [
            thingDiscovered: { DiscoveryService source, DiscoveryResult result ->
                discoveryListenerResult.set result
        ...
        discoveryServiceRegistry.addDiscoveryListener(discoveryListenerMock);
        discoveryServiceRegistry.startScan(new ThingTypeUID(ANY_BINDING_ID_3, ANY_THING_TYPE_3), [
            onFinished: {
                scanListenerResult3.set(Void.TYPE)
            },
            onErrorOccurred: {
            }
        ] as ScanListener)

        waitForAssert({ assertTrue scanListenerResult3.isSet }, 2000)
        assertTrue discoveryListenerResult.isSet

        // 2 discovery services for the same thing type with different bridges - inbox must contain 2 elements
        assertThat inbox.getAll().size(), is(2)
        assertThat inbox.getAll().stream().filter { r -> r.bridgeUID.equals(BRIDGE_UID_1) }.count(), is(1L)
        assertThat inbox.getAll().stream().filter { r -> r.bridgeUID.equals(BRIDGE_UID_2) }.count(), is(1L)

        // should not remove anything
        discoveryServiceMockForBinding3Bridge1.removeOlderResults(discoveryServiceMockForBinding3Bridge1.getTimestampOfLastScan(), discoveryServiceMockForBinding3Bridge1.bridge)
        assertThat inbox.getAll().size(), is(2)

        // should not remove anything
        discoveryServiceMockForBinding3Bridge2.removeOlderResults(discoveryServiceMockForBinding3Bridge2.getTimestampOfLastScan(), discoveryServiceMockForBinding3Bridge2.bridge)
        assertThat inbox.getAll().size(), is(2)

        // start discovery again
        discoveryServiceRegistry.startScan(new ThingTypeUID(ANY_BINDING_ID_3, ANY_THING_TYPE_3), [
            onFinished: {scanListenerResult3.set(Void.TYPE)},
            onErrorOccurred: {
            }
        ] as ScanListener)
        waitForAssert({ assertTrue scanListenerResult3.isSet }, 2000)
        ...
        // 2 discovery services for the same thing type with different bridges - inbox must contain 4 elements
        assertThat inbox.getAll().size(), is(4)
        assertThat inbox.getAll().stream().filter { r -> r.bridgeUID.equals(BRIDGE_UID_1) }.count(), is(2L)
        assertThat inbox.getAll().stream().filter { r -> r.bridgeUID.equals(BRIDGE_UID_2) }.count(), is(2L)
