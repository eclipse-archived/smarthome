angular.module('PaperUI.services', [ 'PaperUI.constants' ]).config(
			...
			$httpProvider.interceptors.push(function($q, $injector) {
				return {
					'responseError' : function(rejection) {
						$injector.get('toastService').showErrorToast(
	...
	return {
		getRenderingModel : function(configParameters, configGroups) {
			...
			for (var i = 0; i < configParameters.length; i++) {
				...
				var group = [];
				if (!parameter.groupName){parameter.groupName = "_default";}
			...
			for (var j = 0; j < groupsList.length; j++) {
				if (groupsList[j].groupName){parameters.push(groupsList[j]);}
			}
			return parameters;
		},
