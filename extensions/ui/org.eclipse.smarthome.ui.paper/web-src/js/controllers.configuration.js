...
angular.module('PaperUI.controllers.configuration', []).controller('ConfigurationPageController',
		function($scope, $location, thingTypeRepository) {
	...
	bindingRepository.getAll();
		...
		function($scope, $mdDialog, thingTypeRepository, bindingRepository, bindingId) {
		...
		'ConfigureBindingDialogController',
		function($scope, $mdDialog, bindingRepository, bindingService, configService, configDescriptionService,
				toastService, bindingId, configDescriptionURI) {
		...
		'ServicesController',
		function($scope, $mdDialog, serviceConfigService, toastService) {
			...
			$scope.remove = function(serviceId, event) {
				var confirm = $mdDialog.confirm().title('Remove configuration').content(
						'Would you like to remove the service configurarion for the service ' + serviceId + '?')
						.ariaLabel('Remove Service Configuration').ok('Remove').cancel('Cancel').targetEvent(event);
		...
		'ConfigureServiceDialogController',
		function($scope, $mdDialog, configService, serviceConfigService, configDescriptionService, toastService,
				serviceId, configDescriptionURI) {
		...
		'GroupController',
		function($scope, $mdDialog, toastService, homeGroupRepository, groupSetupService) {
			...
			$scope.remove = function(homeGroup, event) {
				var confirm = $mdDialog.confirm().title('Remove ' + homeGroup.label).content(
						'Would you like to remove the group?').ariaLabel('Remove Group').ok('Remove').cancel('Cancel')
						.targetEvent(event);
				$mdDialog.show(confirm).then(function() {
	...
	$scope.add = function(label) {
		...
		function($scope, $timeout, $mdDialog, thingRepository, thingSetupService, toastService, homeGroupRepository) {
		...
		'ViewThingController',
		function($scope, $mdDialog, toastService, thingTypeRepository, thingRepository, thingSetupService,
				homeGroupRepository, linkService) {
		...
		function($scope, $mdDialog, toastService, thingSetupService, homeGroupRepository, thing) {
			$scope.thing = thing;
		...
		function($scope, $mdDialog, toastService, itemRepository, linkedItem, acceptedItemType) {
		...
		'EditThingController',
		function($scope, $mdDialog, toastService, thingTypeRepository, thingRepository, thingSetupService,
				homeGroupRepository, configService, thingService) {
			...
			$scope.getThingType = function() {
				thingTypeRepository.getOne(function(thingType) {
					...
					$scope.needsBridge = $scope.thingType.supportedBridgeTypeUIDs
							&& $scope.thingType.supportedBridgeTypeUIDs.length > 0;
