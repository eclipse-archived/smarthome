...
angular.module('PaperUI.controllers.setup', []).controller('SetupPageController',
		function($scope, $location, thingTypeRepository, bindingRepository) {
		...
		'InboxController',
		function($scope, $timeout, $mdDialog, $q, inboxService, discoveryResultRepository, thingTypeRepository,
				thingSetupService, toastService) {
		...
		function($scope, $mdDialog, $q, inboxService, discoveryResultRepository, thingTypeRepository,
				thingSetupService, toastService, thingRepository) {
							...
							inboxService.approve({
								...
								return thingSetupService.setGroups({
									...
									function() {
										...
										toastService.showDefaultToast('Thing added.', 'Show Thing',
												'configuration/things/view/' + thingUID);
			...
			$scope.remove = function(thingUID, event) {
				...
				var confirm = $mdDialog.confirm().title('Remove ' + discoveryResult.label).content(
						'Would you like to remove the discovery result from the inbox?').ariaLabel(
						'Remove Discovery Result').ok('Remove').cancel('Cancel').targetEvent(event);
		...
		function($scope, $rootScope, $timeout, $mdDialog, discoveryService, bindingRepository) {
		...
		function($scope, $mdDialog, discoveryResult, thingTypeRepository, homeGroupRepository) {
		...
		function($scope, bindingRepository, thingTypeRepository, thingSetupService) {
		...
		'ManualSetupConfigureController',
		function($scope, $routeParams, $mdDialog, toastService, bindingRepository, thingTypeRepository,
				thingSetupService, homeGroupRepository, thingRepository, configService) {
	...
	$scope.selectBinding = function(bindingId) {
		...
		'SetupWizardSearchBindingController',
		function($scope, discoveryResultRepository, discoveryService, thingTypeRepository, bindingRepository) {
			$scope.showIgnored = false;
			$scope.toggleShowIgnored = function() {
				$scope.showIgnored = !$scope.showIgnored;
			}
			...
			$scope.filter = function(discoveryResult) {
				return ($scope.showIgnored || discoveryResult.flag === 'NEW')
						&& discoveryResult.thingUID.split(':')[0] === $scope.bindingId;
			}
			$scope.scan = function(bindingId) {
				$scope.scanning = true;
				discoveryService.scan({
