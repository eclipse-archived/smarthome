angular.module('PaperUI.controllers.extension', [ 'PaperUI.constants' ]).controller(
		'ExtensionPageController',
		function($scope, extensionService, bindingRepository, thingTypeRepository, eventService, toastService) {
			...
			eventService.onEvent('smarthome/extensions/*', function(topic, extensionId) {
				...
				if (extension) {
					...
					if (topic.indexOf("uninstalled") > -1) {
						...
						toastService.showDefaultToast('Install or uninstall of extension ' + extension.label
								+ ' failed.');
					}
				}
			});
