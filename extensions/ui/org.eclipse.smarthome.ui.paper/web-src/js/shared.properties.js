	...
	return {
		...
		updateModule : function(type, value) {
			...
			if (!value.id) {
				...
				if (index != -1){modArr[index] = value;}else {
					modArr.push(value);
					tId++;
				}
			}
		...
		removeFromArray : function(opt, id) {
			var arr = null;
			if (angular.equals(opt, "trigger")){arr = triggersArray;}else if (angular.equals(opt, "action")){arr = actionsArray;}else if (angular.equals(opt, "condition")){arr = conditionsArray;}
