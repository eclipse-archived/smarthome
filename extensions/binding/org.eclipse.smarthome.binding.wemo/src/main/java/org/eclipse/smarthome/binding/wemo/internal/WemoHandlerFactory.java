...
public class WemoHandlerFactory extends BaseThingHandlerFactory {
    ...
    protected ThingHandler createHandler(Thing thing) {
        ...
        if (thing.getThingTypeUID().equals(THING_TYPE_BRIDGE)) {
            ...
            WemoBridgeHandler handler = new WemoBridgeHandler((Bridge) thing, upnpIOService);
            registerDeviceDiscoveryService(handler);
            return handler;
        } else if (WemoHandler.SUPPORTED_THING_TYPES.contains(thing.getThingTypeUID())) {
        ...
        } else if (thing.getThingTypeUID().equals(WemoBindingConstants.THING_TYPE_MZ100)) {
            logger.debug("Creating a WemoLightHandler for thing '{}' with UDN '{}'", thing.getUID(),
                    thing.getConfiguration().get(DEVICE_ID));
    ...
    public Thing createThing(ThingTypeUID thingTypeUID, Configuration configuration, ThingUID thingUID,
            ThingUID bridgeUID) {
        if (WemoBridgeHandler.SUPPORTED_THING_TYPES.contains(thingTypeUID)) {
            ThingUID wemoBridgeUID = getBridgeThingUID(thingTypeUID, thingUID, configuration);
            return super.createThing(thingTypeUID, configuration, wemoBridgeUID, null);
        }
        if (WemoHandler.SUPPORTED_THING_TYPES.contains(thingTypeUID)) {
            ThingUID wemoUID = getWemoThingUID(thingTypeUID, thingUID, configuration);
            return super.createThing(thingTypeUID, configuration, wemoUID, null);
        }
        if (WemoLightHandler.SUPPORTED_THING_TYPES.contains(thingTypeUID)) {
            ThingUID wemoLightUID = getLightUID(thingTypeUID, thingUID, configuration, bridgeUID);
            return super.createThing(thingTypeUID, configuration, wemoLightUID, bridgeUID);
        }
        throw new IllegalArgumentException("The thing type " + thingTypeUID + " is not supported by the wemo binding.");
    }

    private ThingUID getWemoThingUID(ThingTypeUID thingTypeUID, ThingUID thingUID, Configuration configuration) {
        if (thingUID == null) {
    ...
    private ThingUID getLightUID(ThingTypeUID thingTypeUID, ThingUID thingUID, Configuration configuration,
            ThingUID bridgeUID) {
        String lightId = (String) configuration.get(DEVICE_ID);
