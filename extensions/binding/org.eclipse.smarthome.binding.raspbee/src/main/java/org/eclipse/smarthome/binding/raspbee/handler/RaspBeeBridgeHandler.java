...
import com.enterprisecoding.jraspbee.Config;
import com.enterprisecoding.jraspbee.FullConfig;
import com.enterprisecoding.jraspbee.FullLight;
import com.enterprisecoding.jraspbee.RaspBeeBridge;
import com.enterprisecoding.jraspbee.State;
import com.enterprisecoding.jraspbee.StateUpdate;
...
/**
 * {@link RaspBeeBridgeHandler} is the handler for a raspbee bridge and connects it to
 * the framework. All {@link RaspBeeLightHandler}s use the {@link RaspBeeBridgeHandler} to execute the actual commands.
 *
 ...
 */
public class RaspBeeBridgeHandler extends BaseBridgeHandler {
    ...
    private Logger logger = LoggerFactory.getLogger(RaspBeeBridgeHandler.class);
        ...
        public void run() {
            try {
                try {
                    ...
                    if (bridge != null) {
                        if (lastBridgeConnectionState) {
                            logger.debug("Connection to RaspBee Bridge {} lost.", bridge.getIPAddress());
                            lastBridgeConnectionState = false;
                            onConnectionLost(bridge);
                        }
                    }
    ...
    private RaspBeeBridge bridge = null;

    public RaspBeeBridgeHandler(Bridge RaspBeeBridge) {
        super(RaspBeeBridge);
    }
    ...
    public void initialize() {
        logger.debug("Initializing raspbee bridge handler.");

        if (getConfig().get(HOST) != null && getConfig().get(PORT) != null) {
            if (bridge == null) {
                bridge = new RaspBeeBridge((String) getConfig().get(HOST), (String) getConfig().get(PORT));
                bridge.setTimeout(5000);
            }
            onUpdate();
        } else {
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.CONFIGURATION_ERROR,
                    "Cannot connect to raspbee bridge. IP address not set.");
        }
    }
            ...
            if (pollingJob == null || pollingJob.isCancelled()) {
                ...
                try {
                    ...
                    if (pollingIntervalConfig != null) {
                        ...
                        logger.info("Polling interval not configured for this raspbee bridge. Using default value: {}s",
                                pollingInterval);
                    }
    ...
    /**
     * This method is called whenever the connection to the given {@link RaspBeeBridge} is lost.
     *
     * @param bridge the raspbee bridge the connection is lost to
     */
    public void onConnectionLost(RaspBeeBridge bridge) {
        logger.debug("Bridge connection lost. Updating thing status to OFFLINE.");
        updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.BRIDGE_OFFLINE);
    }

    /**
     * This method is called whenever the connection to the given {@link RaspBeeBridge} is resumed.
     *
     * @param bridge the raspbee bridge the connection is resumed to
     */
    public void onConnectionResumed(RaspBeeBridge bridge) {
    ...
    private boolean tryResumeBridgeConnection() {
        logger.debug("Connection to RaspBee Bridge {} established.", bridge.getIPAddress());
        if (getConfig().get(USER_NAME) == null) {
            logger.warn("User name for RaspBee bridge authentication not available in configuration. "
    ...
    /**
     * This method is called whenever the connection to the given {@link RaspBeeBridge} is available,
     * but requests are not allowed due to a missing or invalid authentication.
     *
     * @param bridge the raspbee bridge the connection is not authorized
     */
    public void onNotAuthenticated(RaspBeeBridge bridge) {
    ...
    private void createUser(RaspBeeBridge bridge) {
        try {
    ...
    private String createUserOnPhysicalBridge(RaspBeeBridge bridge) throws IOException, ApiException {
        logger.info("Creating new user on RaspBee bridge {} - please press the pairing button on the bridge.",
                getConfig().get(HOST));
        String userName = bridge.link(DEVICE_TYPE);
        logger.info("User '{}' has been successfully added to RaspBee bridge.", userName);
        return userName;
    }
    ...
    private void updateBridgeThingConfiguration(String userName) {
        ...
        try {
            ...
            logger.warn("Unable to update configuration of RaspBee bridge.");
    ...
    private void handleAuthenticationFailure(Exception ex, String userName) {
        logger.warn("User {} is not authenticated on RaspBee bridge {}", userName, getConfig().get(HOST));
    ...
    private void handleLinkButtonNotPressed(LinkButtonException ex) {
        logger.debug("Failed creating new user on RaspBee bridge: {}", ex.getMessage());
        updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.CONFIGURATION_ERROR,
                "Not authenticated - press pairing button on the bridge.");
    }

    private void handleExceptionWhileCreatingUser(Exception ex) {
        logger.warn("Failed creating new user on RaspBee bridge", ex);
        updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.CONFIGURATION_ERROR,
