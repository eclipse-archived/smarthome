...
/**
 * {@link RaspBeeLightHandler} is the handler for a hue light. It uses the {@link RaspBeeBridgeHandler} to execute the
 * actual
 * command.
 *
 ...
 *
 */
public class RaspBeeLightHandler extends BaseThingHandler implements LightStatusListener {
    ...
    private Logger logger = LoggerFactory.getLogger(RaspBeeLightHandler.class);

    // Flag to indicate whether the bulb is of type Osram par16 50 TW or not
    private boolean isOsramPar16 = false;

    private RaspBeeBridgeHandler bridgeHandler;

    ScheduledFuture<?> scheduledFuture;

    public RaspBeeLightHandler(Thing raspBeeLight) {
        super(raspBeeLight);
    }

    @Override
    public void initialize() {
        logger.debug("Initializing raspbee light handler.");
        ...
        if (configLightId != null) {
            ...
            // the bridge
            if (getRaspBeeBridgeHandler() != null) {
    ...
    public void dispose() {
        ...
        if (lightId != null) {
            RaspBeeBridgeHandler bridgeHandler = getRaspBeeBridgeHandler();
            if (bridgeHandler != null) {
                getRaspBeeBridgeHandler().unregisterLightStatusListener(this);
            }
            lightId = null;
        }
    }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        RaspBeeBridgeHandler RaspBeeBridge = getRaspBeeBridgeHandler();
        if (RaspBeeBridge == null) {
            logger.warn("raspbee bridge handler not found. Cannot handle command without bridge.");
            return;
        }

        FullLight light = getLight();
        if (light == null) {
            logger.debug("raspbee light not known on bridge. Cannot handle command.");
            return;
        }
        ...
        if (lightState != null) {
            RaspBeeBridge.updateLightState(light, lightState);
        } else {
    ...
    private synchronized RaspBeeBridgeHandler getRaspBeeBridgeHandler() {
        if (this.bridgeHandler == null) {
            ...
            if (handler instanceof RaspBeeBridgeHandler) {
                this.bridgeHandler = (RaspBeeBridgeHandler) handler;
    ...
    @Override
    public void onLightStateChanged(RaspBeeBridge bridge, FullLight fullLight) {
    ...
    public void channelLinked(ChannelUID channelUID) {
        RaspBeeBridgeHandler handler = getRaspBeeBridgeHandler();
        if (handler != null) {
