...
class AstroOSGiTest extends OSGiTest {
    ...
    protected final String INTERVAL_PROPERTY = "interval"
    protected final BigDecimal INTERVAL_DEFAULT_VALUE = new BigDecimal(300)
    ...
    protected void initialize(String thingID, String channelId, AcceptedItemType acceptedItemType, Configuration thingConfiguration){
        ...
        // If no configuration is returned, NPE in the AbstractDailyJob#sheduleEvent() method is thrown
        astroThing = Mockito.spy(astroThing)
        channel = Mockito.spy(channel)
        when(astroThing.getChannel(anyString())).thenReturn(channel)
