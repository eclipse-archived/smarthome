...
Additionally, it is possible to use OSRAM Lightify devices as well as other ZigBee LightLink compatible products, including the IKEA TRÄDFRI lights (when updated). 
...
If ommited the rule gets triggerd by any key action and you can determine the event that triggered it with the `receivedEvent.getEvent()` method.
Be aware that the events have a '.0' attached to them, like `2001.0` or `34.0`. So, testing for specific events looks like this:_
