...
public class MqttTopicClassMapper {
     ...
     * multiple bean objects registered. All subscriptions will be canceled.
     *
     * @param connection
     */
    public void unsubscribeAll() {
        subscriptions.forEach(c -> connection.unsubscribe(c.topic, c));
        subscriptions.clear();
    }
     ...
     */
    public void unsubscribe(Object objectWithFields) {
        subscriptions.removeIf(m -> {
            boolean same = m.objectWithFields == objectWithFields;
            if (same) {
                connection.unsubscribe(m.topic, m);
            }
            return same;
        });
    }
     ...
     * @param timeout Timeout per subscription in milliseconds. The returned future completes after this time even if no
